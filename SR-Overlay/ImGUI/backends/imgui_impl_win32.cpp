...
IMGUI_IMPL_API LRESULT ImGui_ImplWin32_WndProcHandlerMod(HWND hwnd, WPARAM wParam, LPARAM lParam, bool isMouseInput, RECT clientSize) //if it's not a mouse callback it's a keyboard one
{
    ...
    if (isMouseInput) {
        ...
        case WM_NCMOUSEMOVE:
        {
            ...
            io.AddMousePosEvent((float)mouse_pos.x + clientSize.left, (float)mouse_pos.y + clientSize.top);
        ...
        case WM_SYSKEYDOWN:
            ...
            if (keyboardInfo->vkCode < 512) {
                ...
                const ImGuiKey key = ImGui_ImplWin32_KeyEventToImGuiKey(keyboardInfo->vkCode, lParam);
                const int scancode = (int)keyboardInfo->scanCode;

                // Special behavior for VK_SNAPSHOT / ImGuiKey_PrintScreen as Windows doesn't emit the key down event.
                if (key == ImGuiKey_PrintScreen && !is_key_down)
                    ImGui_ImplWin32_AddKeyEvent(io, key, true, keyboardInfo->vkCode, scancode);

                // Submit key event
                if (key != ImGuiKey_None)
                    ImGui_ImplWin32_AddKeyEvent(io, key, is_key_down, keyboardInfo->vkCode, scancode);

                //wprintf(L"Key details: %d (%d) VK %d SCAN %d \n", key, is_key_down, keyboardInfo->vkCode, scancode);
